...
if [ -d .git ]; then
    ...
    if [ "${CODEBUILD_GIT_ESCAPED_BRANCH}" = "master" ]; then
        export IMAGE_TAG=latest
    else
        export IMAGE_TAG=${CODEBUILD_GIT_ESCAPED_BRANCH}
    fi
    export IMAGE_REPO_NAME=${CODEBUILD_PROJECT}
    
fi

if [ -z ${IMAGE_REPO_NAME} ]; then
    export IMAGE_REPO_NAME=http-sink
fi
